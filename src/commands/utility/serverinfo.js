...
exports.run = async (client, message, args) => {
  ...
  var embed = new Discord.RichEmbed()
    ...
    .addField(`Channels [**${message.guild.channels.size}**]`, `- **${message.guild.channels.filter(m => m.type === 'category').size}** Category \n- **${message.guild.channels.filter(m => m.type === 'text').size}** Text \n- **${message.guild.channels.filter(m => m.type === 'voice').size}** Voice`, true)
    .addField(`Roles [${message.guild.roles.size}]`, `Use **${PREFIX}roles** if you want to see the role list`, true)
